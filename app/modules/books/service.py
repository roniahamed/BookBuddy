...
class BookService:
    ...
    def get_all_reviews(self, pagination: PaginationParams, book_id: int = None, user_id: int = None) -> ReviewPaginatedResponse:
        """All reviews across all users (global feed). Optionally filtered by book_id and user_id."""
        reviews, total = self.repo.get_all_reviews(pagination.offset, pagination.per_page, book_id, user_id)
        items = [
            ReviewResponse(
                id=r.id,
                rating=r.rating,
